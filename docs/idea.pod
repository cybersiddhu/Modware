...
=head3 Take a bread with software testing
...
ModwareX borrowed ideas from datamapper pattern. The adapter represents the datamapper
...
Bio::Chado::Schema. It is splitted into two groups,  reader and writer. Using Moose,
these two layers are combined thereby providing a unified layer to end user. Yes,
ModwareX allows developer to write their own adapter layer. 
...
Figure out the interface you have to implement for the adaptor. The default reader and
writer for Publication module is under ModwareX::Chado::Reader::BCS::Publiction and
ModwareX::Chado::Writer::BCS::Publiction. BCS is the short name of the adaptor. It follows
the format [Module base namespace]::[Reader|Writer]::[Short adaptor name]::[Data module name]

=item 

Implement the interface as Moose::Role for writer and regular perl class for reader. For
reader the methods are class based.

=item

Pass the information to the connector. Now,  assuming you have a reader adapter under
MyBest::Adapter namespace using DBI for publication data module. You would then keep it under ...
MyBest::Adapter::Reader::DBI::Publiction. Then you could set it up like this....
